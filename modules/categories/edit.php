...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($categoryName)) {
        ...
        if ($result->num_rows > 0) {
            ...
            $sql = "UPDATE categories SET category_name = ?, gst_rate=?, is_split=?, sort_order = ?, is_active = ? WHERE id = ?";
            $stmt = $conn->prepare($sql);
            $stmt->bind_param('sdiiii', $categoryName, $gst_rate, $isSplit, $sortOrder, $isActive, $id);
