<?php
ob_start();
$pageTitle = 'Edit Product - Your-boutique';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
	...
	$cat_rate = $cat_rate_Result->fetch_assoc();
        ...
        $gstRate = $cat_rate['gst_rate'];// getGSTRate($mrp);
        
        $sql = "UPDATE products SET product_name = ?, category_id = ?, mrp = ?, gst_rate = ?, is_active = ? WHERE id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param('sidiii', $productName, $categoryId, $mrp, $gstRate, $isActive, $id);
                    ...
                    <div class="row">
                        <div class="col-md-4 mb-3">
                            ...
                            <input type="text" class="form-control" id="product_number" 
                                   value="<?php echo $product['product_number']; ?>" readonly>
                            <small class="text-muted">Cannot edit</small>
                        </div>
