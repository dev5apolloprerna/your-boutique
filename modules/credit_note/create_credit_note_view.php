                        ...
                        <div class="row">
                            <div class="col-md-5 mb-2">
                                <div class="input-group input-group-sm">
                                    <input type="text" class="form-control" id="ex_product_code"
                                        placeholder="Product Code" autocomplete="off">
                                    <button type="button" class="btn btn-outline-primary" id="ex_search_product">
                                        <i class="bi bi-search"></i>
                                    </button>
                                </div>
                                <div class="form-text" id="ex_product_feedback"></div>
                            </div>
                            <div class="col-md-3 mb-2">
                                <select class="form-select form-select-sm" id="ex_size_select" disabled required>
                                    <option value="">Search product first</option>
                                </select>
                            </div>
                            ...
                            <div class="col-md-2 mb-2">
                                <button type="submit" class="btn btn-sm btn-success w-100" id="ex_add_item" disabled>Add</button>
                            </div>
                        </div>
...
<script>
    $(document).ready(function() {
        function resetExchangeProduct(message, isError) {
            $('#ex_product_id, #ex_size_id').val('');
            $('#ex_size_select')
                .html('<option value="">Search product first</option>')
                .prop('disabled', true);
            $('#ex_add_item').prop('disabled', true);
            $('#ex_product_feedback')
                .text(message || '')
        ...
        function searchExchangeProduct() {
            ...
            let code = $('#ex_product_code').val().toUpperCase().trim();
            $('#ex_product_code').val(code);

            if (!code) {
                resetExchangeProduct('Enter a product code.', true);
                return;
            }

            resetExchangeProduct('Searching...', false);
            $.ajax({
                url: baseUrl + '/api/get_product_sizes.php',
                method: 'POST',
                data: {
                    code: code
                },
                ...
                success: function(data) {
                    if (!data.found) {
                        resetExchangeProduct(data.message || 'Product not found.', true);
                        return;
                    }

                    $('#ex_product_id').val(data.id);
                    if (!data.sizes || data.sizes.length === 0) {
                        resetExchangeProduct(data.name + ' has no stock available.', true);
                        return;
                    }
                    ...
                    data.sizes.forEach(function(size) {
                        options += '<option value="' + size.id + '">' +
                            size.name + ' (' + size.quantity + ' available)</option>';
                    });
                    $('#ex_size_select').html(options).prop('disabled', false).focus();
                    $('#ex_product_feedback')
                        .text(data.name + ' — ₹' + parseFloat(data.mrp).toFixed(2))
                        .removeClass('text-danger').addClass('text-success');
                },
                error: function() {
                    resetExchangeProduct('Unable to search for the product.', true);
                }
            });
        }

        $('#ex_search_product').on('click', searchExchangeProduct);
        $('#ex_product_code').on('keydown', function(event) {
            if (event.key === 'Enter') {
                event.preventDefault();
                searchExchangeProduct();
            }
        }).on('input', function() {
            resetExchangeProduct('', false);
        });

        $('#ex_size_select').on('change', function() {
            $('#ex_size_id').val($(this).val());
            $('#ex_add_item').prop('disabled', !$(this).val());
        });

        $('#exchangeForm').on('submit', function(event) {
            if (!$('#ex_product_id').val() || !$('#ex_size_id').val()) {
                event.preventDefault();
                resetExchangeProduct('Search for a product and select an available size.', true);
            }
        });
    });
</script>